...
Speci::Speci(const SpeciInpData& sid) :
    ...
    Vn{static_cast<flt>(id.Vn)},
    Vx{static_cast<flt>(id.Vx)},
...
#pragma omp parallel for num_threads(num_thrds)
        for (unsigned gn = 0; gn < ng; ++gn) {
            ...
            // assign (random) value of V
            V = unV(eng);

            // // NOTE: for testing; variation in starting Q1 (and p) values
            // rand_uni unQ(-0.5, 0.5);
            // for (unsigned i = 0; i < N; ++i) {
            //     gph[i].Q1 = gph[i].Q0 + unQ(eng);
            //     gph[i].p1(bet);
            // }
            ...
            avg_type avg(N, KR, T, T0hist, prwr, V0, V, C,
...
void Speci::h5_read_pop(const std::string& infilename)
{
    ...
    // nHH
    h5.read_uint("nHH", uival);
    for (unsigned i = 0; i < Ntot; ++i) {
        pop[i].phenotype.nHH = uival[i];
    }
    // inum
...
void Speci::h5_write_pop(const std::string& outfilename) const
{
    ...
    std::transform(pop.begin(), pop.end(), uival.begin(),
                   [](const ind_type& i) -> unsigned
                   { return i.phenotype.nHH; });
    h5.write_uint("nHH", uival);
    // inum
    std::transform(pop.begin(), pop.end(), uival.begin(),
                   [](const ind_type& i) -> unsigned
